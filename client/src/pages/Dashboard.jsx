    ...
    <div className="min-h-screen pt-32 pb-20 px-6 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          ...
          <p className="text-white/50 text-lg">Good to see you, {user?.fullname?.split(" ")[0]}!</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <Card className="bg-gradient-to-br from-brand-primary/20 to-transparent border-brand-primary/20">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-white/50 text-sm font-medium">Total Spending</p>
              <h2 className="text-3xl font-bold text-white">₹{totalSpending.toLocaleString()}</h2>
            </div>
          </div>
        </Card>
        <Card>
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-white/50 text-sm font-medium">Active Bills</p>
              <h2 className="text-3xl font-bold text-white">{bills.length}</h2>
            </div>
          </div>
        </Card>
        <Card>
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-white/50 text-sm font-medium">Budget Status</p>
        ...
        <AnimatePresence mode="popLayout">
          {bills.map((bill, index) => (
            ...
            >
              <Card className="flex items-center justify-between group py-4 px-6" hover={true}>
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center text-white/50 group-hover:bg-brand-primary/20 group-hover:text-brand-primary transition-colors">
                    <Calendar className="w-6 h-6" />
                  </div>
                  <div>
                    <h4 className="text-lg font-semibold text-white group-hover:text-brand-primary transition-colors">{bill.purpose}</h4>
                    <p className="text-sm text-white/40">{new Date(bill.date).toLocaleDateString()}</p>
                  </div>
                </div>
                <div className="flex items-center gap-6">
                  <span className="text-xl font-bold text-white">₹{bill.amount.toLocaleString()}</span>
                  <Button
                    variant="danger"
          ...
          {bills.length === 0 && (
            <div className="text-center py-20 bg-white/5 rounded-3xl border border-dashed border-white/10">
              <p className="text-white/40 italic">No bills found. Add your first bill to get started!</p>
            </div>
          )}
        </AnimatePresence>
      </div>
      ...
      <AnimatePresence>
        {isAdding && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center px-6">
            ...
            >
              <Card className="p-8">
                <h2 className="text-2xl font-bold text-white mb-6">Add New Bill</h2>
